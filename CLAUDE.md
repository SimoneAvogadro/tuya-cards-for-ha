...
`binary_sensor.py` and `sensor.py` give their entities `DeviceInfo(identifiers=<ZHA device identifiers>, connections=…)`. Up to HA 2026.7 that **merged** them into the ZHA device. Since HA 2026.8 a device belongs to exactly one config entry (device-registry storage v3; the migration split shared devices on 2026-08-05 on the user's instance), so HA keeps them on a **sibling device** owned by this integration: same identifiers/connections, same `name_by_user`, no switch. Consequences and the rule that follows:

- HA asks *this* integration for device actions on the sibling, and the service device picker (`filter: integration: tuya_irrigation`) lists the sibling. Neither carries the switch, so **every device_id that reaches services or device actions must go through `discovery.resolve_valve_device` / `valve_switch_for_any_device`**, which map a sibling to the ZHA device sharing an identifier or connection (iterating `dev_reg.devices`; `async_get_device(identifiers=…)` is deprecated because identifiers are no longer unique across entries). Forgetting this is exactly the 2026-09-14 bug: "Device … is not a detected irrigation valve" on every scheduled run.
- Discovery (`find_valve_devices`) still keys valves by the ZHA device (switch + volume sensor); the sibling is only ever an *input* to resolve.
- Do not try to merge back into the ZHA device (`async_update_device(add_config_entry_id=…)` is not a merge in v3) and do not parse switch ids out of our unique_ids: a renamed switch leaves a stale entity whose unique_id now points at *another* valve's switch (seen live: `binary_sensor.tze200_a7sghmms_ts0601_irrigating` on the Irrigatore 33 sibling).
