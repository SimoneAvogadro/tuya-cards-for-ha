...
Both services address the valve either by `device_id` (the only field `services.yaml` offers in the UI: a device picker with `filter: integration: tuya_irrigation`, which lists exactly this integration's per-valve sibling devices — see [Device association](#device-association-ha-20268)) or by `switch_entity` (kept in the vol schema for the card, the device actions and pre-2.14 automations; `device_id` wins if both are given). `_switch_from_call` resolves `device_id` via `valve_switch_for_any_device` (accepts the sibling or the ZHA device) and raises `ServiceValidationError` for anything else.
...
- **Entities set no `DeviceInfo`.** They inherit `entity.ValveAttachedEntity` (mix it in first), which in `async_added_to_hass` moves the entity registry entry onto the ZHA device (`ent_reg.async_update_entity(device_id=…)`). What HA reads for device pages, device actions and the service picker is the registry `device_id`, so this puts "Irrigazione in corso", history and water-total on the real valve device and makes `filter: integration: tuya_irrigation` list exactly the ZHA valves. Proven on the same instance by zha-sonoff-quirks' `SwvAttachedEntity`.
