...
- `tuya_irrigation.irrigation_by_seconds(device_id | switch_entity, seconds)` — turn on, wait for the switch to report `on` (bounded by `_OPEN_CONFIRM_TIMEOUT_S`, 10 s: the quirk's clock settle + radio latency would otherwise eat ~1.5 s of every run), `asyncio.sleep(seconds)`, turn off. Cancellation-safe via per-switch task registry.
- `tuya_irrigation.irrigation_by_liters(device_id | switch_entity, liters, timeout_seconds?)` — turn on, monitor `sensor.<prefix>_summation_delivered` via `async_track_state_change_event`, turn off when the target volume is delivered. Two safety nets bound a run that never reaches target: a **stall watchdog** (reason `stalled`, closes after ~`LITERS_STALL_WINDOW_S` of no measured flow) and an **adaptive cap** that samples the flow rate over the first `LITERS_SAMPLE_WINDOW_S` and *tightens* the max runtime toward the estimate — bounded above by `timeout_seconds` (the ceiling, default 3600 s) and `LITERS_HARD_GUARD_S` (absolute backstop); `timeout_seconds` is a **maximum**, never extended. An anomalous close (stall / cap) or a failed valve close raises a `persistent_notification`; an **external** close (card stop button, physical button, native auto-off) ends the monitor silently with no alert.

Both services address the valve either by a standard HA **service target** (`services.yaml` declares `target:` with `device: integration: tuya_irrigation` + `entity: integration: tuya_irrigation, domain: binary_sensor`, so the picker lists exactly the detected valves — see [Device association](#device-association-ha-20268)) or by `switch_entity` (kept in the vol schema for the card and pre-2.14 automations; the target wins if both are given). HA merges `target` into `call.data` before validation, so the schemas take `**cv.ENTITY_SERVICE_FIELDS` (`device_id` / `entity_id` / `area_id` / `floor_id` / `label_id`, string or list); an automation saved with `data: {device_id: …}` keeps working unchanged. `_switch_from_call` resolves the target through `_valve_switches_for_target` (devices via `valve_switch_for_any_device`, entities via their registry device) and raises `ServiceValidationError` when it holds no valve **or more than one** (one valve per action, by design: runs are keyed by switch and alerts name one valve). Declaring `target:` is also what makes the services appear in the automation editor's **"by target" tab** for a valve device: that tab calls the `get_services_for_target` WS command, which matches services' target entity filters against the device's entities (here the "Irrigating" binary_sensor). Device actions cannot do this since HA 2026.8 — they are only asked of the device's *owning* integration (ZHA) — so there is no `device_action.py` any more.
...
Both handlers do only their mode-specific validation and then delegate to the single shared **`_async_begin_run(switch_entity, mode, target, run_coro)`** — so both entry points (card, service call / automation) funnel through the two services, and both services funnel through one start method. `_async_begin_run` runs the start sequence in this exact order: `_cancel_existing` → `managed_switches.add` → `_async_push_run_plan` → `async_create_task(run_coro)` → `_turn_on` (valve opens **last**, after the run-plan DPs are applied, so the device stamps start/end with the target). The device **clock** is not pushed from here: the GiEX quirk in zha-tuya-quirks inserts the Tuya time frame + a 1.5 s settle in front of every valve-open DP, whatever the origin (see [ZHA layer](#zha-layer-zha-tuya-quirks)).
...
## Device association (HA 2026.8+)

Since HA 2026.8 a device belongs to exactly one config entry (device-registry storage v3), so the old trick of claiming the ZHA device through `DeviceInfo(identifiers=<ZHA identifiers>)` no longer merges: it gives this integration a duplicate **sibling** device (same identifiers and name, no switch — the user's instance got them in the 2026-08-05 migration, plus a nameless one for the SONOFF valve adopted before `FOREIGN_VALVE_PLATFORMS` existed). Rules that follow:

- **Entities set no `DeviceInfo`.** They inherit `entity.ValveAttachedEntity` (mix it in first), which in `async_added_to_hass` moves the entity registry entry onto the ZHA device (`ent_reg.async_update_entity(device_id=…)`). What HA reads for device pages, the target picker and `get_services_for_target` is the registry `device_id`, so this puts "Irrigazione in corso", history and water-total on the real valve device and makes `integration: tuya_irrigation` target filters list exactly the ZHA valves. It does **not** make HA ask this integration for device actions (those go only to the device's owning config entry), which is why the services carry a `target:` instead. Proven on the same instance by zha-sonoff-quirks' `SwvAttachedEntity`.
- **`_async_cleanup_registry`** (10 s after setup) removes this integration's entities whose unique_id's switch is not a detected valve (they can never come alive again: unique_ids embed the switch) and deletes devices owned only by this entry that hold no entities. `async_remove_config_entry_device` returns True so leftovers can also be deleted from the UI.
- **Keep `discovery.resolve_valve_device` / `valve_switch_for_any_device`** in front of every device_id that reaches the services: they accept the ZHA device *and* any sibling still referenced by an old automation (same identifiers or connections, iterating `dev_reg.devices`; `async_get_device(identifiers=…)` is deprecated). A deleted sibling in an automation shows as an unknown device and must be re-picked.
- Don't parse switch ids out of unique_ids for anything but the cleanup: a renamed switch leaves a stale entity whose unique_id can name *another* valve's current switch.
...
`sensor.py` exposes two entities per valve, on the same device as `binary_sensor.py` (see [Device association](#device-association-ha-20268)):
...
- **Cover card** (`src/cover-compact-card.js`): one row — icon + name + `Aperto · 21%` stacked on the left, position bar at full card height on the right. The bar is **mirrored**: the fill is anchored to the *right* edge with length `100 − position` (the closed share), so the handle sits at `position`% from the left and dragging right opens; `fill_from: "left"` restores HA's orientation. `cvPosFromX` / `cvBoundaryPct` are exact inverses — change one and the round-trip test in `tests/` catches it. Control is **drag-only** (`CV_DRAG_MIN_PX`, 2 px): a bare tap must never move a shutter, and `cover.set_cover_position` fires once on release. `cvDisplayPos` keeps the commanded value on screen until the device agrees (`CV_PENDING_MS`) so the bar can't snap back while the motor spins up. Icon/name tap → `hass-more-info`. **Colour** comes from `cvStateColorVar`, which rebuilds HA's own fallback chain (`--state-cover-<dc>-<state>-color` → `--state-cover-<state>-color` → `--state-cover-<active|inactive>-color` → `--state-<active|inactive>-color`, `stateActive` for a cover = `state !== "closed"`): **`--state-cover-open-color` does not exist** — the purple is on `--state-cover-active-color`, and skipping it lands on the amber `--state-active-color` (the v1.0.0 bug). **`closed_tolerance`** (default 1) exists because the TS130F runs fully down and then reports `1`, leaving HA `open · 1%`; at or below it `cvEffectivePos` snaps to 0 for the label, the fill, the colour *and* the commanded position. **Icon**: a hand-drawn shutter whose `CV_SLATS` (4) slats are lowered from the top by `cvSlatCount` (one per quarter closed, fed the same `shown` value as the bar, so it follows a drag); mdi has no half-closed shutter, which is why it is not an `ha-icon`. Raised slats stay drawn at opacity .13 so the window outline never disappears. **Layout**: the name column is the fixed element (`flex:0 1 var(--cv-label-width, clamp(120px,44%,220px))`) and the bar is the elastic one — the other way round, each card's bar is only as wide as its name allows and a stack of covers gets visibly different bars. `label_width` overrides it; `cvLabelWidth` accepts only a plain number + unit, since the value lands in a custom property. **Offline**: HA strips `current_position` on `unavailable`, so `cvRememberPos`/`cvRecallPos` keep the last seen position (instance field + `localStorage` under `tuya-cover-pos:<entity_id>`, every access in a try/catch) and the bar still draws it, greyed and locked, with `Offline · 23%` as the label — never store a null, or an outage would erase the memory.
