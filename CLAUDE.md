...
Both services address the valve either by `device_id` (the only field `services.yaml` offers in the UI: a device picker with `filter: integration: tuya_irrigation`, which lists exactly the detected valves because each carries this integration's binary_sensor/sensors) or by `switch_entity` (kept in the vol schema for the card, the device actions and pre-2.14 automations; `device_id` wins if both are given). `_switch_from_call` resolves `device_id` via `valve_switch_for_device` and raises `ServiceValidationError` for a non-valve device.
